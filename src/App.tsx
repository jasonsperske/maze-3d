...
function App() {
    ...
    if (hash) {
      try {
        ...
        if (state.seed && 
          ...
          return {
            seed: state.seed,
            initialPosition: new Vector3(state.position.x, state.position.y, state.position.z),
            initialRotation: new Euler(state.rotation.x, state.rotation.y, state.rotation.z, 'YXZ')
          };
        } else {
    ...
    if (seedParam) {
      ...
      if (storedData) {
        return {
          ...
          initialRotation: storedData.rotation ? new Euler(storedData.rotation.x, storedData.rotation.y, storedData.rotation.z, 'YXZ') : new Euler(0, 0, 0, 'YXZ')
        };
      } else {
  ...
  const printMazeASCII = useCallback((maze: MazeCell[][]) => {
    let output = '';
    ...
    for (let z = 0; z < maze[0].length; z++) {
      ...
      for (let x = 0; x < maze.length; x++) {
        ...
        if (cell.walls.west || x === 0) {
          const hasDoor = doorMap.has(`${x},${z},west`) || (x > 0 && doorMap.has(`${x-1},${z},east`));
          rowLine += hasDoor ? '#' : '|';
        } else {
          rowLine += ' ';
        }
        
        // Cell content - show player position or empty space
        ...
        if (cell.walls.south || z === maze[0].length - 1) {
          const hasDoor = doorMap.has(`${x},${z},south`) || (z < maze[0].length - 1 && doorMap.has(`${x},${z+1},north`));
          bottomLine += hasDoor ? '##' : '--';
        } else {
          bottomLine += '  ';
        }
    ...
    console.log(output);
    return 'Maze printed to console! (* = your position, # = doors)';
  }, [playerPosition, cellSize, seed]);
